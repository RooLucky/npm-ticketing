...
import {
  TicketingBackendRequestError,
  requestTicketingBackend,
} from "@/lib/ticketing/backend";
...
function statusError(response: Response, requestId?: string, upstreamCode?: ErrorCode) {
  ...
  if (response.status >= 500) {
    return ticketingError("INTERNAL_ERROR", "The ticketing request could not be completed", 500, {
      requestId,
    });
  }
...
export async function proxyTicketingRequest<T>(
  ...
  try {
    ...
    return ticketingError(
      "INTERNAL_ERROR",
      "Ticketing is not configured correctly",
      500,
    );
  }
  ...
  try {
    const upstream = await requestTicketingBackend({
      path: options.path,
    ...
    if (error instanceof TicketingBackendRequestError) {
